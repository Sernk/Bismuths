...
namespace Bismuth.Utilities.ModSupport
{
    public static class QuestRegistry
    {
        ...
        public static IQuest? GetNextQuest(Player player, string npcKey)
        {
            return quests.Where(q => q.NpcKey == npcKey && q.IsAvailable(player) && !player.GetModPlayer<QuestPlayer>().CompletedQuests.Contains(q.UniqueKey)).OrderByDescending(q => q.Priority).FirstOrDefault();
        }
