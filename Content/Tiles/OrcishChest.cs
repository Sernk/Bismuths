...
{
    public class OrcishChest : ModTile
    {
        ...
        {
            _ = this.GetLocalization("OrcishChest").Value;
        }
        public override LocalizedText DefaultContainerName(int frameX, int frameY)
        {
            int option = frameX / 36;
            return this.GetLocalization("OrcishChest");
        }
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            if (BismuthWorld.OrcishInvasionStage == 2 || SavingOpenChests.OChest)
            {
                Tile tile = Main.tile[i, j];
                ...
                for (int k = 0; k < 58; k++)
                {
                    if (player.inventory[k].type == ModContent.ItemType<OrcishKey>() && player.inventory[k].stack > 0)              
                    {
                        keySlot = k;
                        break;
                    }
                ...
                if (keySlot == -1 && !SavingOpenChests.OChest) return false;
                if (keySlot != -1) player.inventory[keySlot].stack--;           

                SavingOpenChests.OChest = true;
        ...
        public override void MouseOver(int i, int j)
        {
            ...
            else
            {
                ...
                {
                    if (SavingOpenChests.OChest)
                    {
                        player.cursorItemIconID = ModContent.ItemType<Items.Placeable.OrcishChest>();
                    }
                    else
                    {
                        player.cursorItemIconID = ModContent.ItemType<OrcishKey>();
                    }
