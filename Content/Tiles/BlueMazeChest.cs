...
{
    public class BlueMazeChest : ModTile
    {
        public override void SetStaticDefaults()
        {
            ...
            AddMapEntry(Color.Blue, this.GetLocalization("MapEntry1"), MapChestName);
            DustType = 79;
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            for (int k = 0; k < 58; k++)
            {
                if (player.inventory[k].type == ModContent.ItemType<BlueKey>() && player.inventory[k].stack > 0)
                {
                    keySlot = k;
                    break;
                }
            ...
            if (keySlot == -1 && !SavingOpenChests.BChest) return false;
            if (keySlot != -1) player.inventory[keySlot].stack--;

            SavingOpenChests.BChest = true;
