...
{
    public class BismuthPlayerLayer : PlayerDrawLayer
    {      
        ...
        protected override void Draw(ref PlayerDrawSet drawInfo)
        {
            ...
            if (modPlayer.TribeCurse)
            {
                ...
                Vector2 pos = new Vector2(drawInfo.Position.X + drawPlayer.width / 2f - Main.screenPosition.X,
                                          drawInfo.Position.Y + 350 - 4f - Main.screenPosition.Y);

                DrawData drawData = new DrawData(texture, pos, new Rectangle(0, height * frame, texture.Width, height),
                    new Color(255, 255, 255, 185), 0f, new Vector2(texture.Width / 2f, texture.Height), 1f,
                    drawPlayer.direction == 1 ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0);
            ...
            if (BismuthWorld.OrcishInvasionStage == 1 || modPlayer.OrcishBarTimer > 0)
            {
                ...
                Utils.DrawBorderString(Main.spriteBatch, Language.GetTextValue("Mods.Bismuth.OrcishInvasionName"), emptypos + new Vector2(emptytex.Width / 2 - FontAssets.MouseText.Value.MeasureString(Language.GetTextValue("Mods.Bismuth.OrcishInvasionName")).X * (Language.ActiveCulture.CultureInfo.Name == "ru-RU" ? 0.8f : 1.0f) / 2, 26f), Color.White, Language.ActiveCulture.CultureInfo.Name == "ru-RU" ? 0.8f : 1.0f);
                Utils.DrawBorderString(Main.spriteBatch, Language.GetTextValue("Mods.Bismuth.DefeatedPortals") + BismuthWorld.DefeatedPortals + "/4", emptypos + new Vector2(emptytex.Width / 2 - FontAssets.MouseText.Value.MeasureString(Language.GetTextValue("Mods.Bismuth.DefeatedPortals") + BismuthWorld.DefeatedPortals + "/4").X * (Language.ActiveCulture.CultureInfo.Name == "ru-RU" ? 0.8f : 1.0f) / 2, 108f), Color.White, Language.ActiveCulture.CultureInfo.Name == "ru-RU" ? 0.85f : 1.0f);
            }
            ...
            if (drawPlayer.GetModPlayer<BismuthPlayer>().IsNaga)
            {
                ...
                if (Main.mouseX > raceBar.X && Main.mouseX < raceBar.X + NagaEmpty.Width &&
                    Main.mouseY > raceBar.Y && Main.mouseY < raceBar.Y + NagaEmpty.Height)
                {
                    Utils.DrawBorderString(Main.spriteBatch, Language.GetTextValue("Mods.Bismuth.Wetness") + " " + modPlayer.Wetness + "/100", new Vector2(Main.mouseX + 24, Main.mouseY + 4), Color.White);
                }
            }
            if (drawPlayer.GetModPlayer<BismuthPlayer>().IsVampire)
            {
                ...
                {
                    Utils.DrawBorderString(Main.spriteBatch, Language.GetTextValue("Mods.Bismuth.Hunger") + " " + drawPlayer.GetModPlayer<BismuthPlayer>().Hunger + "/100", new Vector2(Main.mouseX + 24, Main.mouseY + 4), Color.White);
                }
            }
            #region TribeCurseEffect
            if (modPlayer.TribeCurse)
            {
                Texture2D texture = ModContent.Request<Texture2D>("Bismuth/Glow/CurseSkull").Value;
