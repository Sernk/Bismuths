...
namespace Bismuth.Gores
{
    public class SwampWaterDroplet : ModGore
    {
        ...
        public override bool Update(Gore gore)
        {
            ...
            if (gore.frame <= 4)
            {
                ...
                if ((int)gore.frameCounter >= frameDuration)
                {
                    ...
                    if (gore.frame == 5)
                    {
                        int droplet = Gore.NewGore(Main.LocalPlayer.GetSource_FromThis(), gore.position, gore.velocity, gore.type, 1f);
